...
**Public site + publish (Phases 3–4, built 2026-07-20, NOT yet live):** `python -m tools.build_site` assembles the complete deployable site into `publish/` (gitignored staging): landing page with live stats + pricing, free `/sample/` (yesterday's page — roadmap free tier), members-only `/room/` (today's page), `/login/`, terms/privacy, and Cloudflare Pages Functions implementing the full paywall — magic-code email login (KV + HMAC-signed session cookies), Whop webhook membership sync (HMAC-verified), lazy Whop API fallback, admin API for comping access. A **redaction gate** (22 forbidden markers: GEX/gamma vocabulary, firm/broker/account identifiers, ledger/guardrail language — raw HTML incl. CSS) runs on EVERY page; any hit aborts the build. Never loosen the FORBIDDEN list at runtime — only in a reviewed commit. GEX ladder CSS is personal-edition-only (`CSS_GEX`). `python -m tools.publish_public` = build + gate + push to the separate page repo in `publish_config.json` (`remote: null` = dry run — current state; user still owes brand name + hosting decision); wired as a non-fatal final step of `backtest/daily_stats_run.py`. Function source lives in `site/functions_src/` (`_shared.js` is inlined at the `{{LIB}}` marker by the build — edit source, never `publish/functions/`). Local integration test (full auth/paywall/webhook flow, verified green): see GO-LIVE.md, which is the complete switch-flip runbook. The personal edition (GEX/Floor/Ledger) must NEVER reach `publish/` — only `build_site.py` writes there.
