...
**Public publish (Phase 3):** `python -m tools.publish_public` re-renders the `--public` edition, runs a **redaction gate** (scans the raw HTML — CSS included — for 22 forbidden markers: GEX/gamma vocabulary, firm + broker + account identifiers, ledger/guardrail language; any hit aborts), then pushes `index.html` to the separate static-hosting repo configured in `publish_config.json` (tracked; `remote: null` = dry run, which is the state until the user picks the brand/hosting). The staging clone lives in `publish/` (gitignored). Wired as a non-fatal final step of `backtest/daily_stats_run.py`. Never loosen the FORBIDDEN list at runtime — only in a reviewed commit. GEX ladder CSS is personal-edition-only (`CSS_GEX`) so the public page carries zero licensed-feed vocabulary.
